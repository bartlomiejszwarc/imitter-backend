...
router.get('/users/:id/posts', checkIfAuthenticated, (req, res, next) => {
  ...
  //   Post.find({'author._id': req.params.id})
  //     .sort('-date')
  //     .then((documents) => {
  //       res.status(200).json({
...
//DELETING POST - KINDA SECURE
router.delete('/api/posts/:id', checkIfAuthenticated, async (req, res, next) => {
...
router.put('/api/posts/:id', checkIfAuthenticated, (req, res, next) => {
  //DZIAŁA DODAWANIE I USUWANIE LIKÓW; NIE DZIAŁA LICZNIK LIKÓW
  Post.find(
